...
# Initialize Elasticsearch client only if host is provided
if ES_HOST and ES_API_KEY:
    ...
    es = None
    print("Warning: ELASTIC_HOST or ELASTIC_API_KEY not set. Elasticsearch functionality will be disabled.")
...
def search(query: str, k_dense: int = 5, k_sparse: int = 5) -> list[dict]:
    ...
    if es is None:
        print("Elasticsearch not available. Returning empty results.")
        return []
